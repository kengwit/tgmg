...
class tgmg_base {
    public:
        ...
        tgmg_base(S &q_,const int m_,const int n_,const bool x_prd_,const bool y_prd_,V* b_,V* z_)
            ...
            num_l=q.gs_mode<2?(y_prd&&(n&1)?1:0)
                             :(y_prd?nthr_max:nthr_max-1);
            if(num_l>0) {
        ...
        void rat_collapse_x();
        void rat_collapse_y();
        /** Applies Gauss--Seidel sweeps during the second part of the V-cycle.
         * If a symmetric V-cycle is being used, then these sweeps are
         * reversed.
         * \param[in] cyc the number of sweeps to apply.
         * \param[in] symmetric whether a symmetric V-cycle is being used. */
        inline void up_gs_iterations(const int cyc,bool symmetric) {
            if(symmetric) for(int i=0;i<cyc;i++) gauss_seidel_reverse();
            else for(int i=0;i<cyc;i++) gauss_seidel();
        }
    protected:
...
class tgmg_level : public tgmg_base<tgmg_level<V,M>,V,M> {
    public:
        ...
        void print_rat();
        /** Applies Gauss--Seidel sweeps during the first part of the V-cycle,
         * when it is necessary to also set the solution array to zero. If the
         * number of sweeps is non-zero, the optimized gauss_seidel_from_zero
         * routine is applied in place of the first sweep.
         * \param[in] cyc the number of sweeps to apply. */
...
class tgmg : public tgmg_base<S,V,M> {
    public:
        ...
        using tgmg_base<S,V,M>::up_gs_iterations;
        ...
        bool solve(int type,int per_loop,int max_loops,double omega=1,int cyc_down=1,int cyc_up=1,int hcyc_bottom=10);
        bool solve(int type,tgmg_predict &tp,double omega=1,int cyc_down=1,int cyc_up=1,int hcyc_bottom=10);
         ...
         * \param[in] (cyc_down,cyc_up,hcyc_bottom)
         *            configuration parameters to pass to the v_cycle routine.
         * \return True if the tolerance was reached before the maximum number
         * of iterations, false otherwise. */
        inline bool solve_full_multigrid(int cyc_down=1,int cyc_up=1,int hcyc_bottom=10) {
            return solve(3,fmg_per_loop,max_fmg_loops,1,cyc_down,cyc_up,hcyc_bottom);
        }
         ...
         * \return True if the tolerance was reached before the maximum number
         * of iterations, false otherwise. */
        inline bool solve_v_cycle(int cyc_down=1,int cyc_up=1,int hcyc_bottom=10) {
            return solve(4,v_cycle_per_loop,max_v_cycle_loops,1,cyc_down,cyc_up,hcyc_bottom);
        }
         ...
         * \param[in] (cyc_down,cyc_up,hcyc_bottom,cyc_top)
         *            configuration parameters to pass to the v_cycle routine.
         * \return True if the tolerance was reached before the maximum number
         * of iterations, false otherwise. */
        inline bool solve_full_multigrid(tgmg_predict &tp,int cyc_down=1,int cyc_up=1,int hcyc_bottom=10) {
            return solve(3,tp,1,cyc_down,cyc_up,hcyc_bottom);
        }
         ...
         * \return True if the tolerance was reached before the maximum number
         * of iterations, false otherwise. */
        inline bool solve_v_cycle(tgmg_predict &tp,int cyc_down=1,int cyc_up=1,int hcyc_bottom=10) {
            return solve(4,tp,1,cyc_down,cyc_up,hcyc_bottom);
        }
        /** Solves the linear system using symmetric multigrid V-cycles, using
         * the adaptive approach for choosing iterations.
         * \param[in] (cyc_down,cyc_up,hcyc_bottom,cyc_top)
         *            configuration parameters to pass to the v_cycle routine.
         * \return True if the tolerance was reached before the maximum number
         * of iterations, false otherwise. */
        inline bool solve_sym_v_cycle(tgmg_predict &tp,int cyc_down=1,int cyc_up=1,int hcyc_bottom=10) {
            return solve(5,tp,1,cyc_down,cyc_up,hcyc_bottom);
        }

        void v_cycle(int cyc_down=1,int cyc_up=1,int hcyc_bottom=10,bool symmetric=false);
        void fmg_pre(int cyc_down=1,int cyc_up=1,int hcyc_bottom=10,bool symmetric=false);
    ...
    private:
        ...
        void v_cycle_internal(int level,int cyc_down,int cyc_up,int hcyc_bottom,bool symmetric);
        void set_threads(int l,int type,int nthr);
        void do_thread_trial(int l,int type);
        inline double iters_and_error(int type,int iters,double omega,int cyc_down,int cyc_up,int hcyc_bottom) {
            for(int i=0;i<iters;i++) switch(type) {
                case 0: jacobi();break;
                case 1: gauss_seidel();break;
                case 2: sor(omega);break;
                case 3: fmg_pre(cyc_down,cyc_up,hcyc_bottom);
                case 4: v_cycle(cyc_down,cyc_up,hcyc_bottom);break;
                case 5: v_cycle(cyc_down,cyc_up,hcyc_bottom,true);
            }
            return l2_error();
        }
        inline void iters(int type,int iters,double omega,int cyc_down,int cyc_up,int hcyc_bottom) {
            for(int i=0;i<iters;i++) switch(type) {
                case 0: jacobi();break;
                case 1: gauss_seidel();break;
                case 2: sor(omega);break;
